...
const nID = window.location.pathname.split("/")[2]
    ...
    fetch("/api/cms/posts")
      ...
      .then(data => {
        ...
        console.log(data.filter(n => n.id === nID)[0])
        newsTitle.value = data.filter(n => n.id === nID)[0].title
        newsContent.value =  data.filter(n => n.id === nID)[0].content
        publishedOn.value =  data.filter(n => n.id === nID)[0].date.split(" ")[1]
      })
      .catch(err => {
